...
func _player_connected(id):
	...
	var game = load("res://scenes/session.tscn").instance()
	#pong.connect("game_finished",self,"_end_game",[],CONNECT_DEFERRED) # connect deferred so we can safely erase it from the callback
	game.connect("EventGameFinished",self,"_end_game",[],CONNECT_DEFERRED) # connect deferred so we can safely erase it from the callback
